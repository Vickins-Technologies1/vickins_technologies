...
export default function VtixLayout({ children }: { children: React.ReactNode }) {
  ...
  const filteredNav = isOrganizer
    ? navItems
    : navItems.filter((item) => item.href !== "/vtix/dashboard/scanner");
      ...
      >
        <div className="p-4 flex items-center justify-between">
          <div className={`flex items-center gap-2 ${!sidebarOpen && "hidden"}`}>
            <Image src="/logo1.png" alt="V-Tix logo" width={26} height={26} className="h-6 w-6" />
            <h1 className="font-semibold text-lg tracking-tight">V-Tix Africa</h1>
          </div>
          <button
        ...
        <nav className="flex-1 px-3 py-4 overflow-y-auto space-y-6">
          <div className={`${!sidebarOpen && "hidden"} rounded-2xl bg-white/55 p-3`}>
            ...
            <p className="text-xs text-[var(--muted)] mt-1">
              {isOrganizer ? "Organizer" : "Attendee"} access
            </p>
          </div>
          ...
          {sidebarOpen && (
            <div className="space-y-2">
              <div className="rounded-2xl border border-[var(--glass-border)] bg-white/70 px-3 py-3 text-[11px] text-[var(--muted)] leading-relaxed break-words">
                ...
                <p className="mt-2">
                  QR-secured ticketing, instant validation, and encrypted payments.
                </p>
              </div>
        ...
        <header className="h-14 bg-[var(--card-bg)]/80 backdrop-blur-md flex items-center justify-between px-4 sm:px-5 sticky top-0 z-20 shadow-[0_10px_30px_rgba(15,23,42,0.12)]">
          ...
          <div className="flex items-center gap-3 sm:gap-6">
            ...
            <div className="text-right hidden sm:block">
              ...
              <p className="text-xs text-[var(--muted)]">{isOrganizer ? "Organizer" : "Attendee"}</p>
            </div>
          </div>
        </header>
